...
def suma_cubo_pares_for(numeros: Iterable[int]) -> int:
    ...
    for elem in numeros:
        if (elem % 2 == 0):
            numeros_cubo.append(elem ** 3)
    result = 0
    for elem in numeros_cubo:
        result += elem
    return result
...
def suma_cubo_pares_sum_list(numeros: Iterable[int]) -> int:
    ...
    """
    return sum([elem**3 for elem in numeros if elem%2 == 0])
...
suma_numeros_al_cubo_pares =   reduce(lambda num1,num2:num1+num2,numeros_al_cubo_pares)
